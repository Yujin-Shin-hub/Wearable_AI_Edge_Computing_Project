...
/* Includes ---------------------------------------------------------------- */
#include <Edge_AI_Mini_Project_inferencing.h> // 모델 라이브러리
#include <Wire.h>
...
void loop()
{
    ...
    for (size_t ix = 0; ix < EI_CLASSIFIER_DSP_INPUT_FRAME_SIZE; ix += 6) {  // feature 6개라서 +6, 온도까지 포함 => 10
        ...
        imu::Vector<3> gyroVector = bno.getVector(Adafruit_BNO055::VECTOR_GYROSCOPE);  // 센서받음
    ...
    ei_printf(": \n");
    for (size_t ix = 0; ix < EI_CLASSIFIER_LABEL_COUNT; ix++) {   // 라벨 갯수만큼 결과 출력
